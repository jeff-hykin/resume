;(function () {
    ...
    function rewriteImports(code) {
        let tree
        try {
            tree = parse(code, { ecmaVersion: "latest", sourceType: "module" })
        } catch (_) {
            return code
        }
        let patches = []
        ...
        simple(tree, {
            ...
            ImportExpression: (n) => {
                if (n.source.type == "Literal") rewrite(n.source)
            },
        })
        for (let patch of patches.sort((a, b) => b.from - a.from)) code = code.slice(0, patch.from) + patch.text + code.slice(patch.to)
        return code
    }
    ...
    function renderLoggable(value, space, top = false) {
        ...
        if (error) {
            return span(
                ...
                error,
                " ",
                etcButton((e) => expandError(e.target, value))
            )
        } else if (fun) {
    ...
    function expandError(target, val) {
        ...
        for (let fn of parseStack(val.stack)) frames.appendChild(document.createElement("div")).textContent = fn
    ...
    for (let i = 0; i < tabs.length; i++) tabButtons[i].addEventListener("click", () => toggleTab(i))
    document.querySelector("#run").onclick = () => run()
    document.querySelector("#share").onclick = (e) => {
        navigator.clipboard.writeText(document.location.toString().replace(/[#?].*/, "") + "#c=" + encParam(view.state.doc.toString()))
        showNotification(e.target, "URL copied!")
    }
    let exampleSelect = document.querySelector("#examples")
    for (let name of Object.keys(examples)) exampleSelect.appendChild(document.createElement("option")).textContent = name
    ...
    function loadCode(code) {
        let state = EditorState.create({
            doc: code,
            extensions: [keymap.of([{ key: "Ctrl-Enter", run: () => run() }]), basicSetup, javascript()],
        })
        if (view) view.setState(state)
        else view = new EditorView({ state, parent: tabs[0] })
    }
    loadCode(urlCode ? decodeURIComponent(urlCode[1]) : urlBCode ? decParam(urlBCode[1]) : urlExample && examples.hasOwnProperty(decodeURIComponent(urlExample[1])) ? examples[decodeURIComponent(urlExample[1])] : getDefaultCode())
